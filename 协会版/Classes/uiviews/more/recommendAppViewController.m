...
{
	NSString *reqUrl = @"apprecommend.do?param=%@";
	
	NSDictionary *jsontestDic = [NSDictionary dictionaryWithObjectsAndKeys:
								 [Common getSecureString],@"keyvalue",
								 [Common getVersion:OPERAT_RECOMMEND_APP_REFRESH],@"ver",
								 [NSNumber numberWithInt: SITE_ID],@"site_id",
								 [NSNumber numberWithInt: 0],@"platform",
								 [NSNumber numberWithInt: 0],@"order",
								 nil];
...
//网络获取更多数据
-(void)accessMoreService:(int)order
{
	NSString *reqUrl = @"apprecommend.do?param=%@";
...
{
	//重新更新数据
	self.recommendAppItems = (NSMutableArray *)[DBOperate queryData:T_RECOMMEND_APP theColumn:@"" theColumnValue:@""  withAll:YES];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if ([indexPath row] != countItems && countItems != 0){
		...
		if (piclink)
		{
			...
			if (cardIcon == nil)
			{
				UIImage *img = [[UIImage alloc]initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"推荐应用列表默认图片" ofType:@"png"]];
