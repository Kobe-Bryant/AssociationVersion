    ...
    if (self.listArray != nil && self.listArray > 0) {
        ...
        if (ayArray != nil ) {
            ...
            if (img != nil) {
                ...
                UIImage *bgImage = [[UIImage alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"更多默认图片" ofType:@"png"]];
...
- (void)appImageDidLoad:(NSIndexPath *)indexPath withImageType:(int)Type
{
    ...
    if (iconDownloader != nil)
    {
		if(iconDownloader.cardIcon.size.width > 2.0){ 			
            ...
            picView.image = [photo fillSize:CGSizeMake(250, 300)];
...
- (void)accessService
{
    ...
    NSMutableDictionary *jsontestDic = [NSDictionary dictionaryWithObjectsAndKeys:
                                 ...
                                 [NSNumber numberWithInt:[catStr intValue]],@"cats_id",nil];
	
	[[DataManager sharedManager] accessService:jsontestDic command:MORE_CAT_INFO_COMMAND_ID 
								  accessAdress:@"more/ainfo.do?param=%@" delegate:self withParam:jsontestDic];
}
...
- (void)update
{
    ...
    if (self.listArray != nil && [self.listArray count] > 0) {
        PagedFlowView *flowView  = [[PagedFlowView alloc] initWithFrame:CGRectMake(0, 30, 320, 240)];
