...
{	
	...
	if (self.showPicScrollView == nil && self.picArray != nil && pageCount > 0)
	{
		...
		UIScrollView *tmpScroll = [[UIScrollView alloc] initWithFrame:CGRectMake( 0, yValue, self.view.frame.size.width, 460.0f)];
...
- (NSDictionary *)shareSheetRetureValue  // dufu add 2013.04.25
{
    ...
    if (photoUrl.length > 1)
    {
        dict = [NSDictionary dictionaryWithObjectsAndKeys:
                ...
                content,ShareAllContent,
                content,ShareContent,
                photoUrl,ShareUrl, nil];
    }
    else
    {
        dict = [NSDictionary dictionaryWithObjectsAndKeys:
