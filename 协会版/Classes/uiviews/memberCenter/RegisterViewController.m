...
- (void)viewDidLoad {
	...
	UIImage *btnImage = [[UIImage alloc]initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"注册登录按钮" ofType:@"png"]];
	
	UIButton *loginButton = [UIButton buttonWithType:UIButtonTypeCustom];
	loginButton.frame = CGRectMake((320 - btnImage.size.width) * 0.5f, 120, btnImage.size.width, btnImage.size.height);
	...
	[loginButton setTitle:@"完   成" forState:UIControlStateNormal];
	...
	UIImage *sinaBtnImage = [[UIImage alloc]initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"icon_sina" ofType:@"png"]];
	UIButton *sinaButton = [UIButton buttonWithType:UIButtonTypeCustom];
	sinaButton.frame = CGRectMake(CGRectGetMinX(loginButton.frame) + btnImage.size.width / 5 , CGRectGetMaxY(strLabel.frame), sinaBtnImage.size.width, sinaBtnImage.size.height);
	...
	UIImage *tencentBtnImage = [[UIImage alloc]initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"icon_腾讯" ofType:@"png"]];
	UIButton *tencentButton = [UIButton buttonWithType:UIButtonTypeCustom];
	tencentButton.frame = CGRectMake(CGRectGetMaxX(sinaButton.frame) + btnImage.size.width / 5, CGRectGetMaxY(strLabel.frame), tencentBtnImage.size.width, tencentBtnImage.size.height);
    ...
    if (cell == nil) {
		...
		switch (indexPath.row) {
            case 0:
            {
				...
				nameTextField.placeholder = @"输入您的手机号码";
				[nameText release];
            ...
            case 1:
            {
				...
				passwordTextField.placeholder = @"6-20个英文字母或数字";
...
//									 [[weiboArray objectAtIndex:0] objectAtIndex:weibo_user_name],@"reg_name",
//									 [NSNumber numberWithInt:1],@"isWeibo",
//									 @"sina",@"weibo_type",
//									 [[weiboArray objectAtIndex:0] objectAtIndex:weibo_user_id],@"weibo_id",nil];
//		
//		[[DataManager sharedManager] accessService:jsontestDic command:SINAWEI_COMMAND_ID 
//									  accessAdress:@"member/register.do?param=%@" delegate:self withParam:nil];
//	}else{			
//		SinaViewController *sina = [[SinaViewController alloc] init];
//		sina.isRequest = YES;
//		[self.navigationController pushViewController:sina animated:YES];
...
- (void)accessService
{
	NSDictionary *jsontestDic = [NSDictionary dictionaryWithObjectsAndKeys:
								 ...
								 nameTextField.text,@"reg_name",
								 passwordTextField.text,@"reg_pwd",
								 @"0",@"isWeibo",nil];
...
//		{
//			[self performSelectorOnMainThread:@selector(weiboLogin) withObject:resultArray waitUntilDone:NO];
//		}break;
