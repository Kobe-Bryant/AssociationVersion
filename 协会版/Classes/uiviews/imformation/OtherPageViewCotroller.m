...
@implementation OtherPageViewCotroller
...
-(void)addTableView;
{
	...
	UITableView *tempTableView = [[UITableView alloc] initWithFrame:CGRectMake( 0.0f , 0.0f , 320.0f , fixHeight)];
...
{
	IconDownLoader *iconDownloader = [imageDownloadsInProgress objectForKey:[NSString stringWithFormat:@"%d,%@",[catid intValue],indexPath]];
    if (iconDownloader == nil && photoURL != nil && photoURL.length > 1) 
    {
		if ([imageDownloadsInProgress count]>= 5) {
			imageDownLoadInWaitingObject *one = [[imageDownLoadInWaitingObject alloc]init:photoURL withIndexPath:indexPath withImageType:[catid intValue]];
		...
		iconDownloader.imageType = [catid intValue];
        iconDownloader.delegate = self;
        [imageDownloadsInProgress setObject:iconDownloader forKey:[NSString stringWithFormat:@"%d,%@",[catid intValue],indexPath]];
...
- (void) accessService{	
    ...
    {
        NSMutableArray *catItems = (NSMutableArray *)[DBOperate queryData:T_NEWS_CAT theColumn:@"cid" theColumnValue:[NSString stringWithFormat:@"%@",catid] withAll:NO];
        
        if ([catItems count] > 0)
        {
            NSMutableArray *catArray = [catItems objectAtIndex:0];
            catversion = [catArray objectAtIndex:newscat_version];
        }
        else 
        {
            catversion = [NSNumber numberWithInt: 0];
        }
...
//        [outputFormat release];
        ...
        if (cardIcon == nil)
        {
            ...
            picView.image = [img fillSize:CGSizeMake(76, 56)];
            [img release];
            if (self.myTableView.dragging == NO && self.myTableView.decelerating == NO)
            {
                NSString *photoURL = [self getPhotoURL:indexPath];
