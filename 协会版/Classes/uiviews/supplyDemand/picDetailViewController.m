...
- (void)viewDidLoad {
	...
	photoHigh = photoHigh == 0.0f ? 320.0f : photoHigh;
...
{	
	...
	if (self.showPicScrollView == nil && self.picArray != nil && pageCount > 0)
	{
		...
		UIScrollView *tmpScroll = [[UIScrollView alloc] initWithFrame:CGRectMake( -5.0f, yValue, self.view.frame.size.width + 10.0f, 460.0f)];
		...
		for(int i = 0;i < pageCount;i++)
		{
			...
			UIImage *img = [[UIImage alloc]initWithContentsOfFile:
							[[NSBundle mainBundle] pathForResource:@"查看图片详情默认" ofType:@"png"]];
			myiv.image = img;
			[img release];
